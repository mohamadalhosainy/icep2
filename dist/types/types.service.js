"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TypesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const Type_1 = require("./entity/Type");
const typeorm_2 = require("typeorm");
let TypesService = class TypesService {
    constructor(typeRepo) {
        this.typeRepo = typeRepo;
    }
    create(data) {
        const type = this.typeRepo.create(data);
        return this.typeRepo.save(type);
    }
    find() {
        return this.typeRepo.find();
    }
    findOneById(id) {
        if (!id) {
            return null;
        }
        return this.typeRepo.findOne({ where: { id: id } });
    }
    async delete(id) {
        const findType = await this.findOneById(id);
        if (!findType)
            throw new common_1.NotFoundException('Type Not Found');
        try {
            const teachersCount = await this.typeRepo
                .createQueryBuilder('type')
                .innerJoin('type.teachers', 'teacher')
                .where('type.id = :id', { id })
                .getCount();
            const studentsCount = await this.typeRepo
                .createQueryBuilder('type')
                .innerJoin('type.studentTypes', 'studentType')
                .where('type.id = :id', { id })
                .getCount();
            if (teachersCount > 0 || studentsCount > 0) {
                throw new common_1.BadRequestException(`Cannot delete type. It is being used by ${teachersCount} teachers and ${studentsCount} students.`);
            }
            return this.typeRepo.remove(findType);
        }
        catch (error) {
            if (error instanceof common_1.BadRequestException) {
                throw error;
            }
            console.error('Delete type error:', error);
            throw new common_1.BadRequestException('Failed to delete type. It may be in use by other entities.');
        }
    }
    async update(id, data) {
        const findType = await this.findOneById(id);
        if (!findType)
            throw new common_1.NotFoundException('Type Not Found');
        Object.assign(findType, data);
        return this.typeRepo.save(findType);
    }
};
exports.TypesService = TypesService;
exports.TypesService = TypesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(Type_1.TypeEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TypesService);
//# sourceMappingURL=types.service.js.map